...
func _ready() -> void:
	var rule := AlgebraicRule.create(AlgebraicExpression.from_string("(a+0)"),
			AlgebraicExpression.from_string("a"))
	...
	var rule2 := AlgebraicRule.create(AlgebraicExpression.from_string("0"),
			AlgebraicExpression.from_string("(a+-a)"))
